-- Trinité : même contenu que supabase/migrations/20260411120000_trinite_self_notes_manager_reviews_complete.sql
-- Préférez supabase db push (depuis coya-pro/) pour appliquer automatiquement cette migration.
-- Idempotent (réexécutable dans SQL Editor).
--
-- Autres 400 (bailleurs, programme_actions) : apply_bailleurs_contact_column.sql + migrations programme du dossier migrations/, ou db push.

begin;

-- ---------- Notes auto-évaluation (collaborateur) ----------
...
-- ---------- Avis manager (note 1–20 + feedback) ----------
...
-- Rafraîchir le cache API PostgREST (évite l’erreur « table absente du cache schéma »)
